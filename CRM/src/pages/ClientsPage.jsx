...
function validateEmail(value) {
  ...
  if (!trimmed) return "";
  if (!EMAIL_REGEX.test(trimmed)) return "Enter a valid email address (e.g. company@example.com).";
  return "";
}
...
export default function ClientsPage() {
  ...
  const handlePhoneChange = (event) => {
    ...
    setClientForm((current) => ({ ...current, phone: digitsOnly }));
  ...
  const handleClientSubmit = async (event) => {
    ...
    const emailError = validateEmail(clientForm.email);
    if (emailError) errors.email = emailError;

    const clientNameError = validateRequired(clientForm.clientName, "Client name");
    if (clientNameError) errors.clientName = clientNameError;

    const clientEmailError = validateEmail(clientForm.clientEmail);
    if (clientEmailError) errors.clientEmail = clientEmailError;

    if (clientForm.phone) {
      const phoneError = validatePhone(clientForm.phone);
            ...
            <div className="grid gap-5 rounded-[24px] border border-slate-200 bg-slate-50 p-5 md:grid-cols-3">
              <div>
                <TextField
                  ...
                  required
                />
                {formErrors.clientName && <p className="mt-1 text-xs text-rose-600">{formErrors.clientName}</p>}
              </div>
              <div>
                <TextField
                  ...
                  required
                />
                {formErrors.clientEmail && <p className="mt-1 text-xs text-rose-600">{formErrors.clientEmail}</p>}
              </div>
              <TextField

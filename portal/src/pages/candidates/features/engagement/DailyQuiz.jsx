        ...
        <div className="qi-left" ref={leftRef}>
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <p style={{
              ...
              {quiz.durationSeconds} seconds to prove your skills and climb the national leaderboard.
            </p>
          </div>
...
/* ── Results screen ───────────────────────────── */
function ResultsScreen({ score, total, xpEarned, submitError }) {
  const cardRef = useRef(null);
    ...
    <div style={{
      ...
      <div ref={cardRef} style={{
        ...
        <div style={{ padding: '36px 36px 32px', textAlign: 'center' }}>
          ...
          {/* actions */}
          <div className="qr-item" style={{ display: 'flex' }}>
            <Link to="/profile" style={{
              flex: 1, padding: '13px',
...
export default function DailyQuiz() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([quizResponse, rankingResponse]) => {
        ...
        setRanking(rankingResponse?.data?.top10 || []);
      })
      .catch((err) => {
        if (isMounted) {
          if (isRateLimitError(err)) {
            setRateLimitMessage(err.message || 'AI services are temporarily unavailable.');
  ...
  if (loading) {
    return <SkeletonPage variant="default" />;
  }
  ...
  return (
    ...
    <CandidateHeader />
      ...
      {phase === 'results' && (
        <ResultsScreen score={score} total={total} xpEarned={xpEarned} submitError={submitError} />
      )}

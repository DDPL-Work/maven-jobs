    ...
    <button
      ...
      onMouseEnter={(e) => {
        if (variant === "primary") {
          ...
          e.currentTarget.style.boxShadow = `0 6px 18px rgba(0,35,102,.28)`;
        }
        if (variant === "green") {
          e.currentTarget.style.background = C.greenD;
        }
        if (variant === "ghost") {
          e.currentTarget.style.borderColor = "rgba(0,35,102,.3)";
          e.currentTarget.style.color = C.navy;
          e.currentTarget.style.background = "rgba(0,35,102,.04)";
        }
        if (variant === "danger") {
...
function HelpDeskChatForm() {
  ...
  const [chatMessages, setChatMessages] = useState([
    {
      from: "system",
      text: "Welcome to the Help Desk Center! How can we assist you today?",
    },
  ]);
  ...
  function handleChatSend(e) {
    ...
    setTimeout(() => {
      setChatMessages((msgs) => [
        ...
        {
          from: "system",
          text: "Thank you for your message. We will connect with you in a short time!",
        },
      ]);
    }, 800);
  }
  ...
  if (contactMethod === "Chat" && chatStarted) {
    return (
      <div style={{ display: "flex", flexDirection: "column", height: 400 }}>
            ...
            >
              <span
                style={{
                  ...
                  background: msg.from === "user" ? "#002366" : "#e2e8f0",
        ...
        >
          We will connect with you in a short time!
        </div>
      </div>
    );
  }
...
function ProgressSection({
  ...
  const statusCounts = useMemo(() => {
    ...
    for (const s of STATUS_OPTS)
      sc[s] = apps.filter((a) => a.status === s).length;
    return sc;
  }, [apps]);
  ...
  const handleUpdate = async (app) => {
    ...
    try {
      await authService.updateApplicationStatus(app.id, newStatus);
      setProgressBanner({
        type: "success",
        message: "Status updated successfully",
      });
    } catch {
      setProgressBanner({ type: "error", message: "Failed to update status" });
    }
  ...
  const handleResume = async (app) => {
    ...
    try {
      const blob = await authService.previewApplicationResume(app.id);
      const url = URL.createObjectURL(blob);
      window.open(url, "_blank");
    } catch {
      setProgressBanner({ type: "error", message: "Failed to open resume" });
    }
    ...
    <Card className="ep-card" style={{ padding: 0, overflow: "visible" }}>
        ...
        {[
          {
            ...
            val: statusCounts.total,
            color: C.navy,
          },
          {
            label: "Shortlisted",
            val: statusCounts.SHORTLISTED,
            color: C.amber,
          },
      ...
      {progressBanner && (
        <div
          style={{
            padding: "10px 20px",
            background:
              progressBanner.type === "success" ? "#ECFDF5" : "#FEF2F2",
            color: progressBanner.type === "success" ? "#065F46" : "#991B1B",
            fontSize: 13,
            fontWeight: 600,
            borderBottom: `1px solid ${C.s100}`,
          }}
      ...
      <div style={{ overflowX: "auto", width: "100%" }}>
        ...
        >
          ...
          <thead>
            <tr
              style={{ background: C.s50, borderBottom: `1px solid ${C.s100}` }}
            >
              {[
                "",
                "Candidate",
                "Applied role",
                "Status" /* "Update status", "Contact", "Applied on", "Resume" */,
              ].map((h) => (
                <th
                  key={h}
                  style={{
          ...
          <tbody>
            {paginatedApps.length ? (
              paginatedApps.map((app) => {
                const draftVal =
                  draftStatuses[app.id] || app.status || "APPLIED";
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: 34,
                          borderRadius: "50%",
                          background: app.candidateLogoUrl
                            ? `url("${app.candidateLogoUrl}") center/cover no-repeat`
                            : C.navy + "12",
                          display: "flex",
                          alignItems: "center",
                          justifyContent: "center",
                    ...
                    <td style={{ padding: "12px 16px" }}>
                      <div style={{ fontWeight: 700, color: C.s900 }}>
                        {app.candidateName || "Candidate"}
                      </div>
                      <div style={{ fontSize: 12, color: C.s400 }}>
                        {app.candidateCurrentTitle || "Role not set"}
                      </div>
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <div
      ...
      {totalFiltered > 0 && (
        ...
        >
          <div style={{ fontSize: 12.5, color: C.s500, fontWeight: 600 }}>
            Showing <strong>{Math.min(PAGE_SIZE, totalFiltered)}</strong> of{" "}
            <strong>{totalFiltered}</strong> applications
          </div>
          <button
            onClick={() => onViewAll()}
            style={{
...
export default function EmployerProfile() {
  ...
  const defaultTab = initialSession?.role === "RECRUITER" ? "Jobs" : "Overview";
  ...
  useEffect(() => {
    ...
    setAboutDraft({
      ...
      companySize:
        dashboard.company.companySize || dashboard.company.size || "",
      industry: dashboard.company.industry || "",
      perks: Array.isArray(dashboard.company.perks)
        ? [...dashboard.company.perks]
        : [],
    });
  }, [dashboard?.company]);
      ...
      try {
        ...
        setMessages((current) => {
          ...
          return backendThreads.map((thread, index) => {
            ...
            const color =
              previous?.color ||
              CONVERSATION_COLORS[index % CONVERSATION_COLORS.length];
            const avatar =
              thread.candidateAvatar ||
              previous?.avatar ||
              getInitialsFromName(thread.candidateName);
            return {
              ...previous,
              ...thread,
              avatar,
    ...
    const loadDashboard = async () => {
      ...
      try {
        ...
        try {
          const hasApps =
            dashboardData &&
            Array.isArray(dashboardData.applications) &&
        ...
        const nextSession = {
          ...(savedSession || {}),
          companyName:
            response?.data?.company?.name || savedSession?.companyName || "",
          companyId:
            response?.data?.company?.id || savedSession?.companyId || "",
        };
  ...
  useEffect(() => {
    const jobCount = Number(
      dashboard?.tracking?.activeApprovedJobs ??
        dashboard?.company?.activeJobCount ??
        0,
    );
    ...
    const fetchAiCandidates = async () => {
      ...
      try {
        ...
        if (cancelled) return;
        const candidates =
          resp?.data?.data?.candidates || resp?.data?.candidates || [];
        if (candidates.length > 0) {
          setAiTopCandidates(
            candidates.slice(0, 3).map((c, i) => ({
              ...
              match:
                c.matchScore || c.match || Math.round(80 + Math.random() * 15),
              avatar: getInitialsFromName(
                c.name || c.candidateName || "Candidate",
              ),
    ...
    socket.on("connect", () => {
      if (activeConversationIdRef.current) {
        socket.emit("thread:join", {
          threadId: activeConversationIdRef.current,
        });
      }
    });
  ...
  useEffect(() => {
    ...
    chatSocketRef.current.emit("thread:join", {
    ...
    if (remoteVideoRef.current && remoteCallStream) {
      ...
      if (playPromise?.catch) {
        playPromise.catch((err) =>
          console.warn("Remote call video autoplay failed", err),
        );
      }
    }
  ...
  useEffect(() => {
    ...
    const loadThreadMessages = async () => {
      try {
        const response = await authService.getEmployerChatMessages(
          activeConversation.id,
        );
        ...
        setMessages((current) =>
          current.map((conversation) => {
            ...
            return {
              ...
              avatar:
                response?.data?.thread?.candidateAvatar || conversation.avatar,
  ...
  const sendMessage = async () => {
    ...
    try {
      await authService.sendEmployerChatMessage(activeConversation.id, {
        text: outgoing,
      });
    } catch {
    ...
    async (mode) => {
      ...
      try {
        ...
        if (socket?.connected) {
          socket.emit(
            "call:join",
            { threadId: activeConversation.id, mediaType: callType },
            (ack) => {
              if (ack?.ok) {
  ...
  const acceptIncomingCall = useCallback(async () => {
    ...
    const callType =
      activeConversation?.activeCall?.mediaType || callMode || "AUDIO";
    setCallMode(callType);
    setCallStatus("connecting");
    ...
    try {
      ...
      if (socket?.connected) {
        socket.emit(
          "call:answer",
          { threadId: activeConversation.id, answer: { accepted: true } },
          (ack) => {
  ...
  const normalizedJobs = Array.isArray(dashboard?.jobs)
    ? dashboard.jobs
    : dashboard?.jobs?.data || [];
  const normalizedApplications = Array.isArray(dashboard?.applications)
    ? dashboard.applications
    : dashboard?.applications?.data || [];
  ...
  const handleAddEditQuestion = () => {
    const newQ = {
      ...
      options: [],
      maxLength: 500,
      order: editFormData.screeningQuestions.length,
    };
    setEditFormData((prev) => ({
      ...prev,
    ...
    try {
      await authService.employerUpdateJob(selectedJob.id, {
        ...
        screeningQuestions: editFormData.screeningQuestions.map((q, idx) => ({
          _id:
            q._id && q._id.length === 24 && !q._id.startsWith("new_")
              ? q._id
              : undefined,
          question: q.question,
          type: q.type || "TEXT",
  ...
  const handlePreviewResume = async (appId) => {
    try {
      const response = await api.get(
        `/company-panel/applications/${appId}/resume/preview`,
        { responseType: "blob" },
      );
  ...
  const company = {
    ...
    name:
      dashboard?.company?.name || employerSession.current?.companyName || "",
    tagline: dashboard?.company?.tagline || "",
    location: dashboard?.company?.location?.city
      ? [
          dashboard.company.location.city,
          dashboard.company.location.region,
          dashboard.company.location.zone,
        ]
          .filter(Boolean)
          .join(", ")
      : "",
  ...
  const handleReviewShare = async (review) => {
    const reviewId =
      review?.id || review?.reviewId || review?._id || `review-${Date.now()}`;
    const shareToken =
    ...
    const reviewSnapshot = {
      ...
      helpfulCount: Math.max(
        ...
        Number(
          review?.helpfulCount ||
            review?.likes ||
            Math.round((review?.rating || 0) * 17 + 5),
        ),
      ),
    };
    ...
    try {
      localStorage.setItem(
        `maven-review-share:${reviewId}:${shareToken}`,
        JSON.stringify(reviewSnapshot),
      );
      localStorage.setItem(
        `maven-review-share:${reviewId}`,
        JSON.stringify(reviewSnapshot),
      );
  ...
  const activeJobs = Number(
    tracking.activeApprovedJobs ?? dashboard?.company?.activeJobCount ?? 0,
  );
  const overviewCards = [
    {
      label: "Active Jobs",
    ...
    },
    {
      label: "Applications",
      val: Number(
        tracking.totalApplications ?? normalizedApplications.length ?? 0,
      ),
      icon: FiUsers,
      color: C.green,
    },
  ...
  const topMatches = useMemo(() => {
    const mapped = uniqueCandidateApplications.map((application, index) => ({
      ...
      role:
        application.candidateCurrentTitle ||
  ...
  const candidatesTotalPages = Math.max(
    1,
    Math.ceil(candidatesTotal / CANDIDATE_PAGE_SIZE),
  );
  const safeCandidatePage = Math.min(candidatePage, candidatesTotalPages);
  useEffect(() => {
    if (
      prevCandidatesRef.current !== 0 &&
      candidatesTotal !== prevCandidatesRef.current
    )
      setCandidatePage(1);
    prevCandidatesRef.current = candidatesTotal;
  }, [candidatesTotal]);
  const paginatedCandidateApps = uniqueCandidateApplications.slice(
  ...
  const people = paginatedCandidateApps.map((application, index) => ({
    ...
    name: application.candidateName || "Candidate",
    role:
      application.candidateCurrentTitle || application.jobTitle || "Candidate",
  ...
  const candidatePageNumbers = useMemo(() => {
    ...
    for (
      let i = Math.max(2, safeCandidatePage - 1);
      i <= Math.min(tp - 1, safeCandidatePage + 1);
      i++
    )
      p.push(i);
  ...
  const analytics = useMemo(() => {
    ...
    const jobSeries = buildMonthlySeries(
      jobs,
      (job) => job.createdAt || job.updatedAt,
    );
    const applicationSeries = buildMonthlySeries(
      applications,
      (application) => application.appliedAt || application.updatedAt,
    );
    ...
    const totalApplications = Number(
      tracking.totalApplications ?? applications.length ?? 0,
    );
    const activeJobs = Number(
      tracking.activeApprovedJobs ?? dashboard?.company?.activeJobCount ?? 0,
    );
    ...
    const offersSent = applications.filter(
      (application) => application.status === "OFFERED",
    ).length;
    ...
    return {
      ...
      funnelData: [
        ...
        {
          stage: "Shortlisted",
          val: statusCounts.counts[2] || 0,
          color: C.amber,
        },
        {
          stage: "Interview",
          val: statusCounts.counts[3] || 0,
          color: C.indigo,
        },
      ...
      kpis: [
        ...
        },
        {
          label: "Shortlisted",
          val: shortlisted,
          change: `${shortlisted} shortlisted`,
          up: shortlisted > 0,
          color: C.amber,
        },
        {
  ...
  const handleCompanyShare = useCallback(async () => {
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: company.name,
          text: `Check out ${company.name} on MavenJobs`,
          url: shareUrl,
        });
      } else {
        await navigator.clipboard.writeText(shareUrl);
    ...
    try {
      ...
      localStorage.setItem(
        ...
        JSON.stringify({
          ...
          coverImageUrl:
            nextCompany.coverImageUrl || savedSession?.coverImageUrl || "",
        }),
      );
    } catch (error) {
      console.error(error);
    ...
    <>
        ...
        <div style={{ minHeight: "100vh", background: "#f0f4fb" }}>
            ...
            >
                  ...
                  >
                    <div
                      style={{
                        ...
                        opacity: 0.4,
                        background:
                          "linear-gradient(to top, rgba(0,35,102,0.8), transparent)",
                      }}
                    />
                    {/* Edit cover */}
                    {true && (
                  ...
                  <div style={{ padding: "0 24px 20px", position: "relative" }}>
                    ...
                    >
                      <div>
                        ...
                        >
                          {isRecruiter
                            ? employerSession.current?.username
                            : company.name}
                        </h1>
                        {isRecruiter && (
                          <p
                        ...
                        >
                          {isRecruiter
                            ? employerSession.current?.role?.toLowerCase() ||
                              "recruiter"
                            : company.tagline}
                        </p>
                        {!isRecruiter && (
                      ...
                      >
                        ...
                        <Btn variant="ghost" onClick={handleCompanyShare}>
                    ...
                    <div style={{ marginTop: 16 }}>
                      ...
                      >
                        {(company.perks || []).map((p, i) => {
                          const perkIcon = PERK_ICONS.find(
                            (pi) => pi.label === (p.label || p),
                          );
                        ...
                        {people.length > 0 ? (
                          people.map((person) => (
                            <div
                              key={person.id}
                              onClick={() => {
                                setSelectedCandidateApp(person.app);
                              ...
                              onMouseLeave={(e) => {
                                e.currentTarget.style.borderColor = C.s100;
                                e.currentTarget.style.background = C.s50;
                              }}
                            >
                              {person.logoUrl ? (
                    ...
                    {candidatesTotal > CANDIDATE_PAGE_SIZE && (
                      ...
                      >
                        ...
                        >
                          Showing{" "}
                          <strong>
                            {(safeCandidatePage - 1) * CANDIDATE_PAGE_SIZE + 1}-
                            {Math.min(
                              safeCandidatePage * CANDIDATE_PAGE_SIZE,
                              candidatesTotal,
                            )}
                          </strong>{" "}
                          of <strong>{candidatesTotal}</strong>
                        </div>
                        <div
                          style={{
                            display: "flex",
                        ...
                        >
                          <button
                            ...
                            style={{
                              ...
                              border: `1px solid ${C.s200}`,
                              background:
                                safeCandidatePage <= 1 ? C.s50 : "#fff",
                              color: safeCandidatePage <= 1 ? C.s300 : C.s700,
                              display: "flex",
                              alignItems: "center",
                              justifyContent: "center",
                              cursor:
                                safeCandidatePage <= 1 ? "default" : "pointer",
                              fontSize: 14,
                              transition: "all .15s",
                              outline: "none",
                            }}
                            onMouseEnter={(e) => {
                              if (safeCandidatePage > 1) {
                            ...
                            onMouseLeave={(e) => {
                              ...
                              e.currentTarget.style.color =
                                safeCandidatePage <= 1 ? C.s300 : C.s700;
                            }}
                          >
                            <FiChevronLeft size={15} />
                          </button>
                          {candidatePageNumbers.map((n, i) =>
                            n === "..." ? (
                              <span
                                key={`e${i}`}
                                style={{
                              ...
                              <button
                                key={n}
                                onClick={() => setCandidatePage(n)}
                                style={{
                                  width: 34,
                                  height: 34,
                                  borderRadius: 9,
                                  border: `1px solid ${safeCandidatePage === n ? C.navy : C.s200}`,
                                  background:
                                    safeCandidatePage === n ? C.navy : "#fff",
                                  color:
                                    safeCandidatePage === n ? "#fff" : C.s700,
                                  fontWeight: 700,
                                  fontSize: 13,
                                  cursor: "pointer",
                                  display: "flex",
                                ...
                                onMouseLeave={(e) => {
                                  if (safeCandidatePage !== n) {
                                    e.currentTarget.style.borderColor = C.s200;
                                    e.currentTarget.style.color = C.s700;
                                  }
                                }}
                              >
                                {n}
                          ...
                          <button
                            onClick={() =>
                              setCandidatePage((p) =>
                                Math.min(candidatesTotalPages, p + 1),
                              )
                            }
                            disabled={safeCandidatePage >= candidatesTotalPages}
                            style={{
                              width: 34,
                              height: 34,
                              borderRadius: 9,
                              border: `1px solid ${C.s200}`,
                              background:
                                safeCandidatePage >= candidatesTotalPages
                                  ? C.s50
                                  : "#fff",
                              color:
                                safeCandidatePage >= candidatesTotalPages
                            ...
                            onMouseEnter={(e) => {
                              if (safeCandidatePage < candidatesTotalPages) {
                                e.currentTarget.style.borderColor = C.navy;
                                e.currentTarget.style.color = C.navy;
                              }
                            }}
                  ...
                  <Card className="ep-card">
                      ...
                      action={
                        ...
                        >
                          <div style={{ display: "flex", gap: 6 }}>
                            {["all", "urgent", "design", "engineering"].map(
                              (f) => (
                                <button
                                  key={f}
                                  ...
                                  style={{
                                    ...
                                    fontWeight: 700,
                                    cursor: "pointer",
                                    border: `1.5px solid ${jobFilter === f ? C.navy : C.s200}`,
                                    background:
                                      jobFilter === f ? C.navy : "#fff",
                    ...
                    {paginatedJobs.length > 0 ? (
                      paginatedJobs.map((j, i) => {
                        ...
                        const skillSet = Array.isArray(j.skills)
                          ? j.skills
                          : [];
                            ...
                            >
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                >
                                  ...
                                  >
                                    <FiUsers size={11} /> {j.applicantCount}{" "}
                                ...
                                <div style={{ display: "flex", gap: 6 }}>
                                  ...
                                  <button
                                    ...
                                    onMouseOut={(e) => {
                                      e.currentTarget.style.borderColor =
                                        C.s200;
                                      e.currentTarget.style.color = C.s400;
                                    }}
                                  >
                                    <FiEdit2 size={11} />
                                  </button>
                  ...
                  <Card className="ep-card">
                    ...
                    {companyReviews
                      ...
                      .map((u, i) => {
                        ...
                        const reviewerName =
                          ...
                          "Verified Candidate";
                        const reviewerTitle =
                          u.candidateTitle || u.title || u.role || "Candidate";
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              >
                                {[
                                  { icon: FiSmile, color: C.amber },
                                  { icon: FiHeart, color: C.red },
                                  { icon: FiAward, color: C.indigo },
                                  { icon: FiStar, color: C.purple },
                                ].map((e, ei) => (
                                  <span
                                    key={ei}
                                    style={{
                              ...
                              >
                                    ...
                                    {[
                                      ...
                                      <button
                                        ...
                                        onClick={async () => {
                                          ...
                                          try {
                                            await authService.reactToReview(
                                              u.id,
                                              re.id,
                                            );
                                          } catch (_) {}
                                ...
                                <button
                                  onClick={() =>
                                    setShowReactionFor(
                                      showReactionFor === u.id ? null : u.id,
                                    )
                                  }
                                  style={{
                                    display: "flex",
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          Showing page {reviewPage + 1} of {totalReviewPages} ·{" "}
                          {companyReviews.length} total comments
                        </div>
                        <div style={{ display: "flex", gap: 8 }}>
                          ...
                          <Btn
                            ...
                            onClick={() =>
                              setReviewPage((p) =>
                                Math.min(totalReviewPages - 1, p + 1),
                              )
                            }
                            ...
                            style={{
                              padding: "5px 10px",
                              opacity:
                                reviewPage >= totalReviewPages - 1 ? 0.5 : 1,
                            }}
                          >
                            <FiChevronDown
                              style={{ transform: "rotate(-90deg)" }}
                      ...
                      >
                        ...
                        >
                          ...
                          <a
                            href="mailto:rohan@mavenjobs.in"
                            style={{
                              color: C.navy,
                              fontWeight: 700,
                              textDecoration: "none",
              ...
              >
                ...
                <Card className="ep-card" style={{ padding: "18px 18px 14px" }}>
                  ...
                  <Btn
                    variant="ghost"
                    onClick={() => navigate("/employer-dashboard/analytics")}
                    style={{
                      width: "100%",
                      justifyContent: "center",
                      fontSize: 12.5,
                ...
                <Card className="ep-card" style={{ padding: "18px" }}>
                  {activeJobs === 0 ? (
                    <div style={{ textAlign: "center", padding: "20px 10px" }}>
                      ...
                      >
                        Post a job to start receiving AI-matched candidate
                        recommendations.
                      </div>
                      <button
                        onClick={() => navigate("/post-job")}
                          ...
                          <div
                            ...
                            style={{
                              ...
                              borderBottom:
                                i <
                                Math.min(
                                  (aiTopCandidates || topMatches).length,
                                  3,
                                ) -
                                  1
                                  ? `1px solid ${C.s100}`
                ...
                >
                  <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
                    ...
                    <div>
                      ...
                      >
                        Source, screen, and close roles faster
                      </div>
                    </div>
                  </div>
                  <ul
                  ...
                  >
                    {[
                      "Premium resume database access",
                      "AI shortlist with fit reasons",
                      "WhatsApp, email, and chat outreach",
                      "Hiring funnel and source analytics",
                ...
                <Card className="ep-card" style={{ padding: "18px" }}>
                  ...
                  >
                    <span
                      style={{ fontSize: 12.5, color: C.s500, fontWeight: 600 }}
                    >
                      Plan
                    </span>
                    <span
          ...
          {showUpgradeModal && (
            <Modal
              ...
              title="Upgrade to Premium X"
              width={520}
            >
              <div style={{ padding: 12 }}>
                <p style={{ marginTop: 0, fontSize: 15, fontWeight: 700 }}>
                  Access Chat with Candidates
                </p>
                <p style={{ color: C.s600 }}>
                  Messaging candidates is available to Premium X customers only.
                  Upgrade to unlock unlimited outreach and candidate messaging.
                </p>
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                      ...
                      {PERK_ICONS.map((pi, idx) => {
                        ...
                        return (
                          <button
                            ...
                            onMouseEnter={(e) => {
                              if (!alreadyAdded) {
                                e.currentTarget.style.borderColor = pi.color;
                                e.currentTarget.style.background =
                                  pi.color + "0c";
                              }
                            }}
              ...
              >
                <Btn variant="ghost" onClick={() => setShowAboutEditor(false)}>
                  Cancel
                </Btn>
                <Btn
                  variant="primary"
            ...
            <div style={{ display: "flex", height: 520 }}>
              ...
              >
                ...
                {messages.map((conv, i) => (
                  <div
                    key={conv.id}
                    className={`ep-msg-row${i === activeConv ? " active" : ""}`}
                    onClick={() => {
                      setActiveConv(i);
                      setMessages((current) =>
                        current.map((conversation, index) =>
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div
                        style={{
                          fontSize: 12,
                          color: conv.unread ? C.s700 : C.s400,
              ...
              >
                ...
                >
                  <Avatar
                    initials={activeConversation?.avatar || "C"}
                    color={activeConversation?.color || C.navy}
                    size={38}
                    radius={11}
                  />
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      {activeConversation?.from || "Conversation"}
                    </div>
                    <div style={{ fontSize: 12, color: C.s400 }}>
                ...
                {CALLS_ENABLED &&
                  activeConversation?.activeCall?.state === "RINGING" &&
                  activeConversation?.activeCall?.initiatedBy ===
                    "CANDIDATE" && (
                    <div
                      style={{
                        margin: "12px 16px 0",
                    ...
                    >
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ color: C.s500, fontSize: 12 }}>
                          {activeConversation?.activeCall?.mediaType === "VIDEO"
                            ? "Video call"
                            : "Audio call"}
                        </div>
                      </div>
                      <Btn variant="green" onClick={acceptIncomingCall}>
                        Accept
                      </Btn>
                      ...
                      >
                        <div
                          style={{
                            ...
                            border:
                              m.from === "me" ? "none" : `1px solid ${C.s200}`,
          ...
          {CALLS_ENABLED && (
            <Modal
              open={showCall}
              onClose={endCall}
              title={`${callMode === "VIDEO" ? "Video" : "Audio"} Call`}
              width={560}
              noPad
            >
              <div style={{ padding: 20, display: "grid", gap: 16 }}>
                ...
                >
                      ...
                      <div style={{ fontSize: 12.5, color: C.s500 }}>
                        {callStatus === "ringing"
                          ...
                          : callStatus === "connecting"
                            ? "Connecting secure call session"
                            : "Call ready"}
                      </div>
                    </div>
                    {isCallConnected && (
                  ...
                  <Tag color={callMode === "VIDEO" ? C.indigo : C.green}>
                    {callMode}
                  </Tag>
                </div>
                ...
                >
                  {callMode === "VIDEO" ? (
                    ...
                    >
                      ...
                      <video
                        ...
                        style={{
                          ...
                          background: "#000",
                          zIndex: 10,
                          display: localCallStream ? "block" : "none",
                        }}
                      />
                      {!remoteCallStream && (
                        <div
                    ...
                    >
                      ...
                      <div
                        style={{ color: "rgba(255,255,255,.72)", fontSize: 13 }}
                      >
                ...
                >
                  <div style={{ fontSize: 12.5, color: C.s500 }}>
                    The call is negotiated through Socket.IO signaling and
          ...
          >
            ...
            >
              {["overview", "traffic", "pipeline", "sources"].map((t) => (
                <button
                  key={t}
                  className={`ep-ana-tab${anaTab === t ? " active" : ""}`}
                  onClick={() => setAnaTab(t)}
                >
            ...
            >
              {[
                {
                  label: "Active Jobs",
                  val: analytics.kpis[0].val,
                  change: analytics.kpis[0].change,
                  up: analytics.kpis[0].up,
                  color: C.navy,
                },
                {
                  label: "Applications",
                  val: analytics.kpis[1].val,
                  change: analytics.kpis[1].change,
                  up: analytics.kpis[1].up,
                  color: C.green,
                },
                {
                  label: "Shortlisted",
                  val: analytics.kpis[2].val,
                  change: analytics.kpis[2].change,
                  up: analytics.kpis[2].up,
                  color: C.amber,
                },
                {
                  label: "Offers Sent",
                  val: analytics.kpis[3].val,
                  change: analytics.kpis[3].change,
                  up: analytics.kpis[3].up,
                  color: C.indigo,
                },
              ].map((k, i) => (
                <div
                ...
                >
                  ...
                  >
                    {k.up ? <FiArrowUp size={9} /> : <FiArrowDown size={9} />}
                    {k.change} this month
                  </div>
                </div>
              ))}
            ...
            {(anaTab === "overview" || anaTab === "traffic") && (
              <>
                ...
                >
                  {[
                    {
                      label: "Job Posts by Month",
                      data: analytics.monthlyJobs,
                      color: C.navy,
                    },
                    {
                              ...
                              style={{
                                ...
                                background:
                                  r === "30d"
                                    ? chart.color + "14"
                                    : "transparent",
                                color: r === "30d" ? chart.color : C.s400,
                                border: "none",
                                cursor: "pointer",
                                fontFamily: C.fd,
                              }}
                ...
                >
                  ...
                  <BarChart
                    data={analytics.monthlyApplications}
                    colors={analytics.monthLabels.map((_, i) =>
                      i === analytics.monthLabels.length - 1
                        ? C.green
                        : C.navy + "99",
                    )}
          ...
          <Modal
            ...
            title="Premium X - Supercharge Hiring"
            width={760}
          >
            <div
              style={{
            ...
            >
              ...
              <div>
                ...
                >
                  A production-ready recruiting layer for employers who need
                  verified reach, faster screening, stronger employer branding,
            ...
            >
              {[
                ...
                <div
                  ...
                  style={{
                    borderRadius: 14,
                    border: `2px solid ${plan.best ? plan.color : C.s200}`,
                    padding: "18px 16px",
                    position: "relative",
                  ...
                  onMouseEnter={(e) => {
                    ...
                    e.currentTarget.style.boxShadow = `0 12px 32px ${plan.color}18`;
                  }}
                  onMouseLeave={(e) => {
                    e.currentTarget.style.borderColor = plan.best
                      ? plan.color
                      : C.s200;
                    e.currentTarget.style.transform = "";
                    e.currentTarget.style.boxShadow = "";
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ fontSize: 13, color: C.s400 }}>
                      {plan.period}
                    </span>
                  </div>
                  <ul
                  ...
                  >
                    {plan.features.map((f, fi) => (
                      ...
                      >
                        <FiCheckCircle size={13} color={plan.color} />
                        {f}
                      </li>
                    ))}
                  </ul>
                  <Btn
                    variant={plan.best ? "primary" : "ghost"}
                    style={{
                      width: "100%",
                      justifyContent: "center",
                      fontSize: 12.5,
            ...
            <div style={{ borderTop: `1px solid ${C.s100}`, paddingTop: 20 }}>
              ...
              >
                What Premium X adds to the employer dashboard
              </div>
              <div
                style={{
                  display: "grid",
              ...
              >
                {[
                  {
                    icon: FiUserCheck,
                    label: "AI shortlist workspace",
                    desc: "Rank candidates by JD fit, skills, salary, location, and notice period",
                  },
                  {
                    icon: FiMessageSquare,
                    label: "Recruiter outreach desk",
                    desc: "Use chat, email templates, WhatsApp nudges, and follow-up reminders",
                  },
                  {
                    icon: FiBarChart2,
                    label: "Hiring intelligence",
                    desc: "Track source quality, funnel conversion, time-to-shortlist, and offer signals",
                  },
                  {
                    icon: FiAward,
                    label: "Featured employer brand",
                    desc: "Boost company profile, job cards, and recruiter trust badges",
                  },
                  {
                    icon: FiShield,
                    label: "Verified candidate layer",
                    desc: "Prioritize profiles with resume, identity, and skill confidence signals",
                  },
                  {
                    icon: FiCpu,
                    label: "ATS-ready workflows",
                    desc: "Export CSV, attach notes, and prepare integration-friendly pipelines",
                  },
                ].map((f, i) => (
                  <div
                    key={i}
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11.5, color: C.s500 }}>
                        {f.desc}
                      </div>
                    </div>
                  </div>
          ...
          >
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#b45309" }}>
                    Urgent jobs get a badge and higher visibility
                  </div>
                </div>
                <div
                  style={{
              ...
              <div style={{ display: "flex", gap: 10, paddingTop: 4 }}>
                ...
                >
                  <FiBriefcase size={14} /> Publish Job
                </Btn>
              </div>
            </div>
          </Modal>
              ...
              >
                ...
                <div>
                  {(() => {
                    ...
                    const ungrouped = jobApps.filter(
                      (a) =>
                        !statusGroups.some((g) =>
                          g.statuses.includes((a.status || "").toUpperCase()),
                        ),
                    );
                      ...
                      >
                        ...
                        {totalCount === 0 ? (
                          ...
                          >
                            ...
                            <div style={{ fontSize: 12.5, color: C.s400 }}>
                                    ...
                                    >
                                      <div
                                        style={{
                                          ...
                                          background: app.candidateLogoUrl
                                            ? `url("${app.candidateLogoUrl}") center/cover no-repeat`
                                            : `linear-gradient(135deg, ${group.color}18, ${group.color}08)`,
                                      ...
                                      <div style={{ flex: 1, minWidth: 0 }}>
                                        ...
                                        >
                                          <span style={{ color: C.s400 }}>
                                            {app.candidateEmail || "—"}
                                          </span>
                                          {app.candidateCurrentTitle && (
                                            <>
                                              <span style={{ color: C.s300 }}>
                                                |
                                              </span>
                                              <span>
                                                {app.candidateCurrentTitle}
                ...
                >
                  <button
                    ...
                    style={{
                      ...
                      fontFamily: C.fd,
                      cursor: "pointer",
                      boxShadow: "0 4px 14px rgba(0,35,102,.2)",
                      transition: "all .18s",
                    }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.transform = "translateY(-1px)";
                      e.currentTarget.style.boxShadow =
                        "0 6px 20px rgba(0,35,102,.3)";
                    }}
                    onMouseLeave={(e) => {
                      e.currentTarget.style.transform = "";
                      e.currentTarget.style.boxShadow =
                        "0 4px 14px rgba(0,35,102,.2)";
                    }}
                  >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                ...
                <div
                  style={{
                    display: "flex",
                    gap: 0,
                    borderBottom: `1.5px solid ${C.s100}`,
                    marginBottom: 20,
                  }}
                >
                  {[
                    ...
                    <button
                      ...
                      style={{
                        ...
                        fontWeight: 700,
                        fontFamily: C.fd,
                        color: editJobTab === tab.key ? C.navy : C.s400,
                        background: "none",
                        border: "none",
                        borderBottom:
                          editJobTab === tab.key
                  ...
                  >
                      ...
                      >
                        <div>
                          ...
                          <input
                            ...
                            onBlur={(e) =>
                              (e.target.style.borderColor = C.s200)
                            }
                          />
                    ...
                    <div>
                      ...
                      >
                        ...
                        <div>
                          ...
                          <select
                            value={editFormData.isActive ? "Active" : "Closed"}
                            onChange={(e) =>
                              handleEditFormChange(
                                "isActive",
                        ...
                        <div>
                          ...
                          <select
                            value={editFormData.workMode || "On-site"}
                            onChange={(e) =>
                              handleEditFormChange("workMode", e.target.value)
                            }
                            style={{
                              width: "100%",
                              padding: "10px 14px",
                    ...
                    <div>
                      ...
                      >
                        ...
                        {Number(editFormData.salaryMin) > 0 &&
                          Number(editFormData.salaryMax) > 0 &&
                          Number(editFormData.salaryMax) <
                            ...
                            >
                              Maximum salary should be greater than minimum
                              salary.
                            </div>
                          )}
                      </div>
                ...
                {editJobTab === "screening" && (
                  ...
                  >
                    ...
                    >
                      ...
                      />
                      Changes to screening questions will only apply to new
                      candidates. Existing candidates' answers are preserved.
                    </div>
                    ...
                    {editFormData.screeningQuestions.length === 0 && (
                      ...
                      >
                        ...
                        <div style={{ fontSize: 12.5 }}>
                          Click "Add Question" to create your first screening
                          question.
                        </div>
                      </div>
                    )}
                    ...
                    <button
                      ...
                      onMouseEnter={(e) => {
                        e.currentTarget.style.background = C.navy + "0c";
                        e.currentTarget.style.borderColor = C.navy + "50";
                      }}
                      onMouseLeave={(e) => {
                        e.currentTarget.style.background = C.navy + "06";
                      ...
                      <>
                        <span
                          style={{
                            ...
                            borderTopColor: "#fff",
                            animation: "rmSpinEdit .6s linear infinite",
                          }}
                        />{" "}
                        Saving...
                      </>
        ...
        >
          ...
          >
            ...
            >
              {deletePassStep === 0
                ? "Delete account"
                : `Step ${deletePassStep} of 2 � ${deletePassStep === 1 ? "Enter password" : "Re-enter password"}`}
            </h3>
            <p
              style={{
                fontSize: 13,
            ...
            <div style={{ display: "flex", gap: 10 }}>
              ...
              <button
                ...
                disabled={
                  deleting ||
                  (deletePassStep === 1 && !deletePassword) ||
                  (deletePassStep === 2 && !deletePassword2)
                }
                style={{
                  flex: 1,
                  padding: "12px",
      ...
      {showCandidateModal &&
        ...
        (() => {
          ...
          const screening =
            Array.isArray(a.screeningAnswers) && a.screeningAnswers.length > 0
              ? a.screeningAnswers
              : [];
          const fmtDate = (v) => {
            if (!v) return "-";
                  ...
                  >
                    <div
                      style={{
                        ...
                        border: `1px solid ${C.s200}`,
                        flexShrink: 0,
                        background: a.candidateLogoUrl
                          ? `url("${a.candidateLogoUrl}") center/cover no-repeat`
                          : C.s100,
                      }}
                    >
                      {!a.candidateLogoUrl &&
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 13, color: C.s500 }}>
                        {a.candidateEmail || a.candidatePhone
                          ? `${a.candidateEmail || ""}${a.candidateEmail && a.candidatePhone ? " · " : ""}${a.candidatePhone || ""}`
                          : ""}
                      </div>
                    </div>
                  </div>
                ...
                >
                  ...
                  >
                    {[
                      ...
                      {
                        label: "Status",
                        val: prettifyStatus(a.status || "APPLIED"),
                        color: statusColors[a.status] || C.s500,
                      },
                      {
                        label: "Applied On",
                        val: fmtDate(a.appliedAt),
                        color: C.s600,
                      },
                      {
                        label: "Experience",
                        val: a.candidateExperience || "—",
                        color: C.s600,
                      },
                      {
                        label: "Location",
                        val: a.candidateCity || a.candidateState || "—",
                        color: C.s600,
                      },
                      {
                        label: "Current Title",
                        val: a.candidateCurrentTitle || "—",
                        color: C.s600,
                      },
                    ].map((info, i) => (
                      <div
                  ...
                  <div
                    style={{
                      ...
                      padding: "16px 18px",
                      borderRadius: 14,
                      background: `linear-gradient(135deg, ${C.navy}08, ${C.navy}02)`,
                      border: `1px solid ${C.navy}18`,
                      display: "flex",
                      alignItems: "center",
                      gap: 14,
                  ...
                  {screening.length > 0 && (
                    <div>
                      ...
                      >
                        <FiFileText size={16} color={C.navy} /> Screening
                        Questions &amp; Answers
                        <span
                          style={{
                            fontSize: 12,
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {cropModal.kind === "logo" ? "Crop Logo" : "Crop Cover Image"}
              </div>
              <button
                onClick={() => {
                  if (!cropSaving) {
            ...
            <div
              style={{
                position: "relative",
                width: "100%",
                height: cropModal.kind === "logo" ? 380 : 280,
                background: "#000",
              }}
            >
              <Cropper
                ...
                style={{
                  ...
                  cropAreaStyle: {
                    border:
                      cropModal.kind === "logo" ? "none" : "2px solid #fff",
                  },
                }}
              />
            </div>
            ...
            >
              ...
              <button
                ...
                disabled={!croppedPixels || cropSaving}
                style={{
                  padding: "9px 20px",
                  borderRadius: 10,
                  border: "none",
                  ...
                  cursor: "pointer",
                  fontFamily: C.fd,
                  opacity: !croppedPixels || cropSaving ? 0.6 : 1,
                  display: "flex",
                  alignItems: "center",
                  gap: 6,
                }}

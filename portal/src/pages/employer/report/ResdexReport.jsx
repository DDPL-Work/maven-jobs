...
export default function ResdexReport() {
  ...
  // Company Users — fetched dynamically from the current company
  const [companyUsers, setCompanyUsers] = useState([]);
  const [selectedUserIds, setSelectedUserIds] = useState([]);
  const [userDropdownOpen, setUserDropdownOpen] = useState(false);
  const userDropdownRef = useRef(null);

  // Fetch all users of the current company on mount
  useEffect(() => {
    userManagementService.getUsers().then((res) => {
      const users = (res?.data || []).map((su) => ({
        id: su.userId || su.id,
  ...
  const { data: contactedCandidates = [], isLoading: isContactedLoading } =
    useContactedCandidates(activeTab === "contacted-candidate-mis");
      ...
      try {
        ...
        if (stored) {
          try {
            ...
            if (isMounted)
              setCompany({ name: parsed.companyName || "My Company" });
          ...
          if (backendSub) {
            if (backendSub.subscriptions) {
              setEmailSubscriptions((prev) => ({
                ...prev,
                ...backendSub.subscriptions,
              }));
            }
            if (
              Array.isArray(backendSub.emailList) &&
              backendSub.emailList.length > 0
            ) {
  ...
  const handleSaveAutoEmail = async () => {
    ...
    try {
      ...
      localStorage.setItem(
        "maven_resdex_subscriptions",
        JSON.stringify(emailSubscriptions),
      );
      showToast(
        currentSub === "disabled"
  ...
  const handleSendNow = async () => {
    ...
    try {
      ...
      showToast(
        res?.message ||
          `Excel report sent successfully to ${validEmails.join(", ")}!`,
      );
    } catch (err) {
  ...
  const exportToExcel = useCallback(
    (headers, rows, filename) => {
      ...
      showToast("Report exported successfully to Excel!");
    },
    [showToast],
  );
  ...
  const handleOneClickGenerate = async () => {
    ...
    try {
      ...
      const durationStr = formatResdexDuration(
        res.from || new Date(),
        res.to || new Date(),
      );
      ...
      setGeneratedReport({
        ...
        headers: headers.length
          ? headers
          : ["Subuser", "Total CV Views", "Total Searches"],
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      let finalHeaders = headers && headers.length > 0 ? headers : ["Subuser", "Activity"];
      let finalRows = rows && rows.length > 0 ? rows : [["No data recorded for this period", 0]];
      ...
      if (displayFormat === "excel") {
        ...
        showToast(
          `${TABS.find((t) => t.id === activeTab)?.label} generated successfully!`,
        );
      }
    } catch (err) {
                      ...
                      <div className="rxr-form-row">
                        ...
                        <div className="rxr-row-content">
                          ...
                          >
                            ...
                            >
                              <span>
                                {selectedUserIds.length === companyUsers.length
                                  ? `${companyUsers.length} Users selected`
                            ...
                            {userDropdownOpen && (
                              <div className="rxr-multiselect-dropdown">
                                ...
                                >
                                  ...
                                  />
                                  <span>
                                    Select All ({companyUsers.length} Users)
                                  </span>
                                </div>
                                  ...
                                  <label className="rxr-checkbox-label">
                                    <input
                                      ...
                                      onChange={(e) =>
                                        setDbFields({
                                          ...dbFields,
                                          resumesForwarded: e.target.checked,
                                        })
                                      }
                                    />
                                    Resumes forwarded
                            ...
                            <div className="rxr-row-content">
                              <label className="rxr-radio-label">
                                <input
                                  ...
                                  onChange={() =>
                                    setUsageSubscriptionType("active")
                                  }
                                />
                                <span className="rxr-radio-custom" />
                                ...
                                <label className="rxr-checkbox-label">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setCallFields({
                                        ...callFields,
                                        cvViewsAppPercent: e.target.checked,
                                      })
                                    }
                                  />
                                  % of CV Views on App
                                ...
                                {/* <label className="rxr-checkbox-label">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      setCallFields({
                                        ...callFields,
                                        callsInitiated: e.target.checked,
                                      })
                                    }
                                  />
                          ...
                          {activeTab !== "comments-reports" && (
                            <div style={{ width: "100%" }}>
                              <div className="rxr-helper-note">
                                Note: The data is available for the maximum of
                                last 12 months. The reports can be generated
                                upto a day before the present day.
                              </div>
                            </div>
                          )}
                        </div>
                          ...
                          <div className="rxr-row-content">
                            ...
                            >
                              ...
                              <input
                                ...
                                onChange={(e) =>
                                  setSearchFilterKeyword(e.target.value)
                                }
                              />
                            </div>
                          </div>
              ...
              <div className="rxr-contacted-container">
                ...
                <div className="rxr-contacted-breadcrumb">
                  <div className="rxr-contacted-folder-icon">
                    ...
                    >
                      <path
                        d="M1.5 4C1.5 2.61929 2.61929 1.5 4 1.5H8.5C9.32843 1.5 10.0883 1.90827 10.5528 2.59271L11.7236 4.30729C12.1881 4.99173 12.948 5.4 13.7764 5.4H20C21.3807 5.4 22.5 6.51929 22.5 7.9V16C22.5 17.3807 21.3807 18.5 20 18.5H4C2.61929 18.5 1.5 17.3807 1.5 16V4Z"
                        fill="#0073e6"
                      />
                      <circle cx="12" cy="10" r="2" fill="white" />
                      <path
                        d="M8.5 15C8.5 13.067 10.067 11.5 12 11.5C13.933 11.5 15.5 13.067 15.5 15H8.5Z"
                        fill="white"
                      />
                    </svg>
                  </div>
                  ...
                  <span className="rxr-contacted-crumb-active">
                    Contacted Candidates in last 7 days
                  </span>
                </div>
                ...
                <div className="rxr-contacted-header-row">
                  ...
                  <div className="rxr-contacted-page-controls">
                    ...
                    <button
                      type="button"
                      className="rxr-contacted-page-nav-btn"
                      disabled
                      title="First page"
                    >
                      &laquo;
                    ...
                    <div className="rxr-contacted-page-box">Page 1 of 1</div>
                    <button
                      type="button"
                      className="rxr-contacted-page-nav-btn"
                      disabled
                  ...
                  <div className="rxr-contacted-toolbar-left">
                    <label className="rxr-contacted-check-label">
                      <input
                        ...
                        onChange={(e) =>
                          setContactedSelectAll(e.target.checked)
                        }
                      />
                    ...
                    <button
                      ...
                      onClick={() =>
                        showToast(
                          "Select at least one candidate to add to folder.",
                        )
                      }
                    >
                    ...
                    <button
                      ...
                      className="rxr-contacted-toolbar-btn disabled"
                      onClick={() =>
                        showToast("Select candidates to schedule a reminder.")
                      }
                    >
                ...
                {isContactedLoading ? (
                  ...
                  <div className="rxr-contacted-empty-card">
                    ...
                    >
                      ...
                      />
                      <path
                        d="M73 40C73 35 97 35 97 40L102 52H68L73 40Z"
                        fill="#3b82f6"
                        opacity="0.85"
                      />
                      <circle cx="85" cy="53" r="2.5" fill="#fef08a" />
                      <polygon
                        points="65,54 105,54 125,120 45,120"
                        fill="#60a5fa"
                        opacity="0.1"
                      />
                      ...
                      <path
                        d="M102 68C102 62.5 106.5 58 112 58H122C127.5 58 132 62.5 132 68C132 73.5 127.5 78 122 78H115L108 84V78H112C106.5 78 102 73.5 102 68Z"
                        fill="#ffffff"
                        stroke="#93c5fd"
                        strokeWidth="1.5"
                      />
                      ...
                      <path
                        d="M126 65C126 60 142 60 142 65C138 64 130 63 126 65Z"
                        fill="#0f172a"
                      />
                      <path
                        d="M124 77C124 75 144 75 144 77L146 102H122L124 77Z"
                        fill="#0284c7"
                      />
                      <path d="M130 76L134 82L138 76" fill="#ffffff" />
                      <path
                        d="M124 82L116 98L122 100"
                        stroke="#0284c7"
                        strokeWidth="3"
                        strokeLinecap="round"
                      ...
                      />
                      <path
                        d="M123 102L121 138H145L144 102H123Z"
                        fill="#1e293b"
                      />
                      <line
                        x1="128"
                      ...
                      <path
                        d="M48 108C48 105 50 103 53 103H72L78 108H118C121 108 123 110 123 113V152C123 155 121 157 118 157H53C50 157 48 155 48 152V108Z"
                        fill="#2563eb"
                      />
                      ...
                      <path
                        d="M32 156C32 156 30 148 26 146C22 148 24 156 32 156Z"
                        fill="#60a5fa"
                      />
                      <path
                        d="M34 155C34 155 35 146 41 144C43 148 39 154 34 155Z"
                        fill="#3b82f6"
                      />
                      <path
                        d="M33 157C33 150 33 142 33 138"
                        stroke="#2563eb"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                      />
            ...
            {(activeTab === "database-usage" ||
              ...
              <div className="rxr-card">
                ...
                >
                  ...
                  {activeTab === "search-report" &&
                    "Auto Emailing of Search Reports"}
                  {activeTab === "user-login" &&
                    "Auto Emailing of Login Reports"}
                </h2>

                {/* Notification Banner */}
                <div
                  className={`rxr-status-banner ${emailSubscriptions[activeTab] === "disabled" ? "not-subscribed" : "subscribed"}`}
                >
                  {emailSubscriptions[activeTab] === "disabled" ? (
                    ...
                    <>
                      ...
                      <span>
                        Subscribed (
                        {emailSubscriptions[activeTab].toUpperCase()} delivery)
                      </span>
                    </>
                  )}
                </div>
                  ...
                  >
                    ...
                    {(activeTab === "database-usage" ||
                      ...
                      <label className="rxr-radio-label">
                        ...
                        />
                        <span className="rxr-radio-custom" />
                        Monthly (Previous Month Report delivered on 1st of every
                        Month)
                      </label>
                    )}
                    ...
                    <label className="rxr-radio-label">
                      ...
                      <span className="rxr-radio-custom" />
                      {activeTab === "database-usage" &&
                        "Disable RESDEX Usage MIS alerts"}
                      {activeTab === "search-report" &&
                        "Disable RESDEX Search MIS alerts"}
                      {activeTab === "user-login" &&
                        "Disable RESDEX Login MIS alerts"}
                      {activeTab === "contacted-candidate-mis" &&
                        "Disable Contacted MIS alerts"}

        ...
        {candidate.answers && candidate.answers.length > 0 && (
          <>
            <div style={{ fontSize: '14px', fontWeight: 800, color: '#002366', textTransform: 'uppercase', letterSpacing: '1.5px', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '10px', marginTop: '10px' }}>
              Application Q&A <div style={{ height: '2px', background: '#e2e8f0', flex: 1 }}></div>
            </div>
            {candidate.answers.map((ans, i) => (
              <div key={i} style={{ marginBottom: '20px' }}>
                <div style={{ fontSize: '14px', fontWeight: 800, color: '#0f172a', marginBottom: '6px' }}>Q: {ans.question}</div>
                <div style={{ fontSize: '13px', color: '#475569', paddingLeft: '12px', borderLeft: '3px solid #10b981', lineHeight: 1.5 }}>A: {ans.answer}</div>
              </div>
            ))}
          </>
        )}
    ...
    <div ref={ref} style={{ width: '794px', height: '1123px', background: '#fff', fontFamily: "'DM Sans', sans-serif", color: '#1e293b', padding: '50px 60px', position: 'relative' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '40px' }}>
        ...
        <div>
          ...
          {candidate.answers && candidate.answers.length > 0 && (
            <>
              <div style={{ fontSize: '15px', fontWeight: 800, color: '#0f172a', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '20px', marginTop: '10px' }}>Application Q&A</div>
              {candidate.answers.map((ans, i) => (
                <div key={i} style={{ marginBottom: '20px' }}>
                  <div style={{ fontSize: '14px', fontWeight: 800, color: '#0f172a', marginBottom: '6px' }}>Q: {ans.question}</div>
                  <div style={{ fontSize: '13.5px', color: '#475569', paddingLeft: '14px', borderLeft: '3px solid #10b981', lineHeight: 1.6 }}>A: {ans.answer}</div>
                </div>
              ))}
            </>
          )}
...
export const TemplateCorporateClassic = forwardRef(({ candidate }, ref) => {
  ...
  return (
    <div ref={ref} style={{ width: '794px', height: '1123px', background: '#fff', fontFamily: "'DM Sans', sans-serif", color: '#334155', padding: '45px 55px' }}>
      ...
      {candidate.answers && candidate.answers.length > 0 && (
        <div style={{ marginBottom: '25px' }}>
          <h2 style={{ fontSize: '16px', fontWeight: 800, color: '#000', textTransform: 'uppercase', borderBottom: '1px solid #ccc', paddingBottom: '4px', marginBottom: '12px' }}>Application Q&A</h2>
          {candidate.answers.map((ans, i) => (
            <div key={i} style={{ marginBottom: '16px' }}>
              <div style={{ fontSize: '13px', fontWeight: 800, color: '#000', marginBottom: '6px' }}>Q: {ans.question}</div>
              <div style={{ fontSize: '13px', color: '#333', paddingLeft: '12px', borderLeft: '2px solid #ccc', lineHeight: 1.5 }}>A: {ans.answer}</div>
            </div>
          ))}
        </div>
      )}

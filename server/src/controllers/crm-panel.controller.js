...
const validateDepartment = (value) => {
  ...
  return DEPARTMENT_REGEX.test(trimmed);
};

const validateTerritory = (value) => {
  const trimmed = String(value || "").trim();
  if (!trimmed) return true;
  return TERRITORY_REGEX.test(trimmed);
};

const validatePhone10Digit = (value) => {
  const trimmed = String(value || "").trim();
  if (!trimmed) return true;
...
exports.createClient = asyncHandler(async (req, res) => {
  ...
  if (!validateEmail(email)) {
    throw createHttpError(400, "Enter a valid company email address (e.g. company@example.com).");
  }

  if (!validateEmail(clientEmail)) {
    throw createHttpError(400, "Enter a valid client email address (e.g. client@example.com).");
  }

  if (phone && !validatePhone10Digit(phone)) {
...
exports.updateClient = asyncHandler(async (req, res) => {
  ...
  if (req.body.phone && !validatePhone10Digit(req.body.phone)) {
    throw createHttpError(400, "Phone number must contain exactly 10 digits.");
  }
...
exports.getCandidateProfile = asyncHandler(async (req, res) => {
  ...
  if (candidateProfile) {
    candidateProfile.profileViews = (candidateProfile.profileViews || 0) + 1;
...
exports.updateApplicationStatus = asyncHandler(async (req, res) => {
  ...
  const statusChanged = prevStatus !== application.status;
...
exports.updateSettings = asyncHandler(async (req, res) => {
  if (req.body.fullName && !String(req.body.fullName || "").trim()) {
    throw createHttpError(400, "Full name cannot be empty.");
  }
  ...
  if (req.body.department && !validateDepartment(req.body.department)) {
    throw createHttpError(400, "Department can only contain letters, spaces, hyphens, and ampersands.");
  }

  if (req.body.territory && !validateTerritory(req.body.territory)) {
    throw createHttpError(400, "Territory can only contain letters, spaces, and hyphens.");
  }

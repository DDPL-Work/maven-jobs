...
exports.getPayments = asyncHandler(async (req, res) => {
  ...
  if (role && ["CANDIDATE", "CLIENT"].includes(role.toUpperCase())) {
    matchFilter.role = role.toUpperCase();
  }
  ...
  const [
    ...
    PaymentTransaction.aggregate([
      { $match: { status: "PAID", role: "CANDIDATE" } },
      { $group: { _id: null, total: { $sum: "$amount" } } },
    ]),
    PaymentTransaction.aggregate([
      { $match: { status: "PAID", role: "CLIENT" } },
      { $group: { _id: null, total: { $sum: "$amount" } } },
    ]),
    User.countDocuments({ "membership.plan": "PRO" }),
    User.countDocuments({ "membership.plan": "ELITE" }),
  ]);
  ...
  res.status(200).json({
    ...
    data: {
      transactions: transactions.map((t) => ({
        ...
        companyName: t.companyId?.name || null,
        role: t.role,
      ...
      revenue: {
        ...
        thisMonth: monthRev[0]?.total || 0,
        thisYear: yearRev[0]?.total || 0,
        byCandidates: candidateRev[0]?.total || 0,
        byEmployers: employerRev[0]?.total || 0,
      },
      activeMembers: {
        pro: proCount,
        elite: eliteCount,
